...
`webfetch` 面向本地个人 Agent 的按需网页读取场景，用于把单个 URL 转换成 Agent 可消费的结构化 JSON 和 Markdown。
...
- 输出 Markdown、纯文本、链接拓扑、图片、代码块、结构化数据、元数据、候选评分与告警。
